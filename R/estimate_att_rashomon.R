#' Rashomon-Intersection Estimate of the Average Treatment Effect on the Treated (ATT)
#'
...
#'   Default: 0.
#' @param max_leaves Optional integer. Passed to \code{cross_fitted_rashomon}. Restricts Rashomon set to trees with at most this many leaves.
...
#' @param ... Additional arguments passed to optimaltrees via \code{cross_fitted_rashomon}.
#'
#' @details
...
#'
#' @return List with elements: theta (point estimate from the shared-intersection
...
#' @keywords internal
#' @export
estimate_att_rashomon <- function(X, A, Y, K = 5, outcome_type = c("binary", "continuous"),
  ...
  # candidate splits before the leaf-count penalty ever gets to reject them. Default 4L
  # matches estimate_att_crossfit so the two paths are symmetric (see MEMORY
  # estimate-att-depth-cap-asymmetry). Callers can pass max_depth = 0L for unlimited.
  ...
  # not covered by the valid-inference theory; warn when requested.
  if (isTRUE(auto_tune_intersecting)) {
    warning(
      ...
      "estimate_att_crossfit() if the intersection is empty.",
      call. = FALSE
    )
  }
  ...
  ci_95_wald <- .att$ci_95          # plain Wald interval of the display estimate
  ...
  # fully-fold-specific path (estimate_att_crossfit) the twin IS the estimator, so no
  # honesty correction is needed and the plain Wald interval stands.
  ...
  list(
    theta = theta,
    sigma = sigma,                 # honest twin SE
    ci_95 = ci_95,                 # honest bias-aware CI
    ci_95_wald = ci_95_wald,       # plain Wald interval of the display estimate
    theta_crossfit = theta_crossfit,  # fully-fold-specific twin
