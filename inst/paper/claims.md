...
|---|---|---|---|---|---|
| `ass:sparsity` | Assumption | **Drafted 2026-09-21, audited and fixed the same session** (`manuscript.tex:123`). Structural sparsity, $\cS_e\ne\emptyset$ and $\cS_\mu\ne\emptyset$ — formalizes what was previously six unlabeled prose restatements into one `\ref`-able assumption, placed after `def:sufficient`. Four now cite it by label (`:266,:292,:363,:493`); `lem:single-tree-linear` (`:717`, needs only $\cS_\mu\ne\emptyset$) is genuinely one-sided and correctly left as direct prose, but `lem:selection` had to be restated per-$j$ to make that citation legitimate (`domain-reviewer` found the original joint hypothesis was stronger than the separable selection mechanism needs). `cor:single-tree-coarsening` (`:744`) turned out **not** to be one-sided — its two hypotheses conjoin to exactly this assumption — so it now says so explicitly rather than restating in isolation. Also fixed in the same pass: a false "strictly weaker than sparsity" claim about `ass:pseudo` (sparsity's generic non-singleton $\cS_j$ is an exact tie, so `ass:pseudo` is non-nested with this assumption, not weaker than it) and two overclaims in the new legibility-vs-audit-trail remark. Full detail: `quality_reports/reviews/2026-09-21_ass-sparsity.md`. The single most load-bearing condition in the paper; every efficiency claim below rests on it. | `manuscript.tex:123` | S1 (`partition_recovery_clt`)'s DGPs are constructed to satisfy this by design | **Drafted, audited, fixed** |
...
| *(legibility-vs-audit-completeness remark, no label — prose)* | Remark | **Drafted 2026-09-21** (`manuscript.tex:139`, titled "Legibility does not by itself make the audit trail complete"). Distinguishes legibility (delivered by the fixed cutpoint grid and leaf budget regardless of whether `ass:sparsity` holds) from a *complete* audit trail (displayed tree exactly equals the truth — requires `ass:sparsity`), and forward-points to `sec:honest-manuscript`'s anchor interval for the failure case. Placed immediately after `ass:sparsity` (not at the earlier-planned `:88`, which predates `ass:sparsity` existing to reference) for a clean backward citation. | `sec:trees`, `manuscript.tex:139` | `ass:sparsity` (backward reference, by label) | **Drafted** |
