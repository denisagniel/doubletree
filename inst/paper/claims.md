...
| `prop:spectest` | Proposition | Fidelity diagnostic: size $\to0$, consistent under separated-margin alternative | 602 | **None found** | **GAP** — statement now cites `ass:pseudo`/`prop:pseudo-consistency` by label (2026-09-17 edit) instead of an inline, unlabeled hypothesis; evidentiary status unchanged, still no simulation |
...
| *(ATT-vs-ATE scope, no label — prose)* | Scope decision | **New, decided 2026-09-18.** ATT stays the paper's primary estimand throughout. The main machinery (`lem:selection`-style selection consistency, the CLT, `prop:bilinear`'s bilinear decomposition, the anchor interval) is asserted to generalize to the ATE — not yet verified end-to-end, since it rests on the EIF's bilinear structure being estimand-agnostic. **Explicitly NOT generalized:** the single-tree corollary family (§3.5: `cor:single-tree`, `lem:single-tree-linear`, `cor:single-tree-saturated`, `cor:single-tree-coarsening`, `rem:single-tree-se`) and `rem:two-trees-why`'s propensity-informativeness justification — both lean on `hahn1998`'s asymmetry (knowing $e_0$ exactly lowers the efficiency bound for the ATT, not the ATE) and stay ATT-only by decision, not oversight. ATE-specific results go in a new Appendix (currently empty); a Discussion remark will revisit the choice. | New — Appendix (not yet drafted), Discussion (not yet drafted) | None — scope decision, no numbered result yet | **Decided, not yet drafted** |
