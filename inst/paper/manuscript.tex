...
\label{cond:P}
For a nuisance $j\in\{e,\mu\}$, write $\tauhat_j=(\hat T_j,\hat c_j)$ for its
topology and threshold vector. Condition~(P) holds for $j$ if:
\begin{enumerate}[label=(P\arabic*),leftmargin=*]
\item[(P0)] There exists a fixed reference partition
$\tau_{0,j}=(T_{0,j},c_{0,j})\in\cT_{\Lbar}$ on which $\gamma_{0,j}$ is
$\Prob$-a.s.\ constant, with every leaf of $\tau_{0,j}$ of positive
$\nu$-mass;
\item[(P1)] $\Prob\bigl(\hat T_j=T_{0,j}\bigr)\longrightarrow1$;
\item[(P2)] on the event in (P1), the set $S_{j,n}$ of observations whose
leaf assignment under $\hat c_j$ differs from their assignment under
$c_{0,j}$ satisfies $\card{S_{j,n}}=O_p(1)$.
\end{enumerate}
\end{definition}
...
functions. When $\cS_j$ (Definition~\ref{def:sufficient}) contains more than
one sufficient partition -- as can happen on a fixed grid with redundant
cutpoints -- clause (P1) should be read as convergence to \emph{a} partition
inducing the same leaf-wise expansion as $\tau_{0,j}$, not necessarily a
literal unique limit: Instantiation~1 below in fact verifies Condition~(P)
by a uniformity argument over the whole of $\cS_j$, precisely because a
grid-exact sufficient class need not be a singleton. Instantiation~2 has a
topologically unique target by construction (the genuine-jump and
separation clauses of the sparsity assumption used there rule out ties), so
...
uniformly bounded; $\Lbar$ is fixed; and Condition~(P) holds for both
$j=e,\mu$. Then, with both tree structures and leaf values fitted using all
$n$ observations,
\[
...
Both instantiations below prove Theorem~\ref{thm:main} by the same strategy,
even though the technical work verifying Condition~(P) differs: anchor the
estimator at the fixed reference partition $\tau_{0,j}$, where the usual
fixed-partition expansion for a sufficient nuisance applies directly, and
then show that the perturbation from $\tau_{0,j}$ to $\tauhat_j$ reassigns
only $O_p(1)$ observations, which is enough (given only a finite second
moment on $Y$, per Remark~\ref{rem:P2-mechanism}) for the resulting
disturbance to $\sqrt n(\thetahat-\theta_0)$ to vanish. In both instantiations $V$ is exactly
$\E[\psi_0^2]$, the \emph{same} variance as if the true partition were known
in advance: learning the thresholds, whether confined to a fixed grid or
continuum-valued, costs nothing asymptotically. Full proofs are in the
companion theory document.
...
the sufficient class, topology recovery is exact and there are no
continuum-valued thresholds left to estimate -- Condition~(P) holds with (P2)
satisfied trivially.
...
We now verify Condition~(P), by an argument that differs from the
...
\begin{proposition}[Condition~(P) holds under grid-exact sparsity]
\label{prop:P-instantiation1}
Suppose the conditions of Proposition~\ref{lem:uniform} hold and
$\cS_e\ne\emptyset$, $\cS_\mu\ne\emptyset$. Then Condition~(P) holds for both
$j=e,\mu$: (P0) holds by hypothesis, since a nonempty $\cS_j$ contains a
grid partition on which $\gamma_{0,j}$ is exactly constant, with every leaf
of positive mass by construction of the grid atoms
(\S\ref{sec:trees}); (P1) is the conclusion of
Proposition~\ref{lem:selection} applied at the level of leaf-wise
expansions (see the remark following Definition~\ref{cond:P}); and (P2)
holds trivially, because a grid-restricted partition's thresholds are
exactly its cutpoints: once $\tauhat_j\in\cS_j$, no observation is ever
reassigned between $\tauhat_j$ and $\tau_{0,j}$, so $S_{j,n}=\emptyset$ and
$\card{S_{j,n}}=0=O_p(1)$ identically. Theorem~\ref{thm:main} therefore applies, its proof
routed through Proposition~\ref{lem:uniform}'s uniformity over $\cS_j$
rather than a single-topology anchor.
\end{proposition}
...
For each nuisance $\gamma_{0,j}$, $j\in\{e,\mu\}$: (S) \emph{Tree
...
fixed depth $d$; (J) \emph{Genuine jumps} -- every true split separates
regions whose population values differ by an amount between fixed constants
$0<\kappa_{\min}\le\kappa_{\max}<\infty$; (Dens) \emph{Local connected
density} -- the covariate law has a density bounded between fixed positive
constants on a fixed neighborhood of every true splitting boundary, on
supports whose relevant boxes have bounded diameter; (Sep) \emph{Separation
and interiority} -- distinct true boundaries on the same coordinate are
separated by a fixed $\varsigma>0$, and every true boundary is at least
$\varsigma$ from the support boundary; (Node) \emph{Positive node mass} --
every true node and leaf has $\nu$-mass at least a fixed $q_{\min}>0$;
(Local-Jump) \emph{Jumps across every active face cell} -- every pair of
leaves adjacent across a true boundary differs in population value by
between $\kappa_{\min}$ and $\kappa_{\max}$; (Joint) \emph{Ancestor--
descendant joint density} -- covariates on distinct splitting coordinates
have a jointly bounded density near any ancestor--descendant pair of true
boundaries.
\end{assumption}

Assumption~\ref{ass:sparsity-jump}'s jump and separation clauses (J) and
(Sep) are what rule out topological ties and give continuum sparsity a
\emph{unique} true partition, unlike the grid-exact case of
\S\ref{sec:instantiation1}.

\begin{assumption}[Two-stage solver fidelity]
\label{ass:solver-twostage}
The two-stage fitting procedure -- Stage~A, an exact global minimizer of the
penalized empirical risk over an inflated-budget tree class built on a
search grid of resolution $r_n\to\infty$, followed by Stage~B, a sequence of
exact one-threshold scans, root-first, over the observed covariate values
in each recovered node's own bracket -- satisfies: Stage~A and Stage~B both
use all $n$ observations, with no sample splitting; Stage~B refines
threshold locations only, adding, deleting, or recoordinating no split;
every recovered bracket contains the corresponding true threshold with
probability tending to one; both stages use deterministic, measurable
...
Under Assumptions~\ref{ass:causal}, \ref{ass:construct}, and
\ref{ass:sparsity-jump}, and a search-grid resolution $r_n\to\infty$ growing
slowly enough relative to $n$ (with the minimum-leaf floor $m_n=o(n/r_n)$),
the collapsed Stage-A topology is, with probability tending to one, in
bijection with the true boundaries of $\gamma_{0,j}$, each recovered
boundary within $O(r_n^{-1})$ of its true match, for each $j\in\{e,\mu\}$.
\end{proposition}

\begin{proposition}[Threshold superconsistency, restated from the companion paper]
\label{prop:twostage-superconsistency}
Under Assumption~\ref{ass:sparsity-jump} and~\ref{ass:solver-twostage}, and
on the topology-recovery event of Proposition~\ref{prop:boundary-recovery},
every Stage-B-refined threshold satisfies $\hat c_{j,k}-c_{0,j,k}=O_p(n^{-1})$,
simultaneously over $k=1,\ldots,K_j\le\Lbar-1$, for each $j\in\{e,\mu\}$.
\end{proposition}
...
\ref{ass:sparsity-jump}, and~\ref{ass:solver-twostage}, Condition~(P) holds
for both $j=e,\mu$: (P0) is Assumption~\ref{ass:sparsity-jump}(S), with
positive leaf mass from (Node); (P1) is
Proposition~\ref{prop:boundary-recovery}'s topology-recovery conclusion; and
(P2) follows from Proposition~\ref{prop:twostage-superconsistency}'s
$O_p(n^{-1})$ rate: by the same order-statistic argument underlying
Remark~\ref{rem:P2-mechanism}, this threshold rate, together with the local
density bounds of Assumption~\ref{ass:sparsity-jump}(Dens), implies
$\card{S_{j,n}}=O_p(1)$. Theorem~\ref{thm:main} therefore applies with the
\emph{same} variance $V=\E[\psi_0^2]$ as Instantiation~1, via the
anchor-and-perturbation argument of \S\ref{sec:partition-recovery}.
\end{proposition}
