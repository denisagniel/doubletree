...
itself (\S3.1) -- that the true nuisances be \emph{exactly} representable by
some tree within the budget $\Lbar$ on the analyst's own grid. We do not
minimize this cost: fixing the grid in advance means the truth must align
with it exactly, not merely approximately, for Theorem~\ref{thm:main}'s
guarantees to hold with no further adjustment. This cost can be quantified
further for continuous covariates split at a fixed set of institutional
thresholds: companion theoretical work bounds the gap between the best
interpretable approximation at the analyst's own grid and the best possible
approximation over an unrestricted tree of the same size by an explicit term
governed by the grid's mesh width, but that bound is informative only for
much finer grids than the handful of institutionally motivated cutpoints
(age brackets, income bands, clinical cutoffs) this paper's examples use --
at those coarser grids the bound is vacuous relative to a trivial ceiling.
We report this for completeness rather than as reassurance: refining the
grid does not, by itself, relax the requirement of structural sparsity.

The reason this is not fatal to the paper's inferential guarantees is
Theorem~\ref{thm:anchor}: validity does not require structural sparsity to
hold, and it does not even require the anchor to be built from the same
interpretable machinery. The anchor need only be some valid, $\sqrt
n$-consistent estimator (\S3.2); it could be a standard AIPW estimator with
