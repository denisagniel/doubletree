...
\textbf{Key insight:} We do not require that $s^*$ appears in all $M$ structures (Rashomon stability), nor that $s^*$ recovers a unique oracle structure. Instead, we prove that \emph{any} selected structure is near-oracle in risk---hence its predictions are near-oracle via the loss--norm link---and then show that averaging $M$ refits of $s^*$ produces functionally consistent predictions. This enables rigorous quantification: for observations $i, j$ with $X_i = X_j$, we can bound $|\mubar(X_i) - \mubar(X_j)|$ with high probability and provide formulas for choosing $M$ to achieve target tolerance $\varepsilon$ (e.g., 2\%). Structure \emph{identity} is neither claimed nor needed; when several partitions have near-equal risk the selected structure need not stabilize, yet predictions still converge.
...
The standard approach fixes one structure but has prediction variability (leaf values differ by fold). M-split has both near-oracle risk of the selected structure (Theorem~\ref{thm:structure-consistency}) and prediction consistency ($|\mubar(X_i) - \mubar(X_j)| < \varepsilon$ for $X_i = X_j$, Theorem~\ref{thm:functional-consistency}).
...
\subsection{Theorem 1: Near-Oracle Risk of the Selected Structure}

\begin{theorem}[The selected structure is near-oracle in risk]
\label{thm:structure-consistency}
Let $R(s) = \E[L(Z, f_s(X))]$ denote the population risk of the best leaf-value fit on structure $s$, and let $s_0 = \argmin_s R(s)$ be the oracle optimal structure among trees of complexity $s_n = n^{d/(2\beta+d)}$. Let $\shat_m$ denote the structure selected on split $m$ via doubletree (Rashomon intersection or penalized minimization), and let $s^*$ be \emph{any} selection rule returning one of $\shat_1,\dots,\shat_M$ (e.g.\ the mode, or lowest cross-validated risk). Under Assumptions~\ref{assump:smoothness}--\ref{assump:independence}:

\textbf{(a) Per-split near-oracle risk.} For each split $m$,
\[
R(\shat_m) - R(s_0) = O_p\!\left(n^{-2\beta/(2\beta+d)}\right) \xrightarrow{p} 0
\quad\text{as } n\to\infty.
\]

\textbf{(b) Selected structure inherits the rate.} Because $s^*$ is one of the $\shat_m$ and $M=o(n)$,
\[
R(s^*) - R(s_0) = O_p\!\left(n^{-2\beta/(2\beta+d)}\right).
\]

\textbf{(c) Prediction closeness (via the loss--norm link).} Consequently, with the loss--norm constant $C_{\mathrm{ln}}$ of Assumption~\ref{assump:regularity} (established in Lemma~\ref{lem:loss-norm}),
\[
\|f_{s^*} - f_{s_0}\|_{L^2(\Prob)}^2 \;\le\; C_{\mathrm{ln}}\,\bigl(R(s^*)-R(s_0)\bigr) \;=\; O_p\!\left(n^{-2\beta/(2\beta+d)}\right).
\]
\end{theorem}

\begin{proof}
\textbf{Proof roadmap:} We do \emph{not} claim $s^*$ recovers the identity of $s_0$; when several structures induce near-equal population risk (no separating margin), the selected structure need not stabilize. We claim only that any selected structure has near-oracle \emph{risk}, and hence near-oracle \emph{predictions} via the loss--norm link---which is all Theorems~\ref{thm:pointwise-convergence} and \ref{thm:inference} use.

\paragraph{Step 1: Per-split near-oracle risk (part a).}
By Assumption~\ref{assump:tree} and the oracle inequality underlying it (manuscript Theorem~2; \citealp{xuStatisticalOptimalityOptimal2026}, Theorem~3.1 for squared error, Theorem~\ref{thm:ce-oracle} for cross-entropy), the penalized minimizer on split $m$ satisfies, with probability tending to one,
\[
R(\shat_m) - R(s_0) \;\le\; C\,\frac{s_n \log(n d)}{n} \;=\; O_p\!\left(n^{-2\beta/(2\beta+d)}\right),
\]
using $s_n = n^{d/(2\beta+d)}$ and $2\beta/(2\beta+d) < 1$. This is a statement about excess \emph{risk}, which the oracle inequality controls directly; it does \emph{not} require the minimizer of the empirical risk to coincide with $s_0$ as a combinatorial object. This is the step where the earlier version over-claimed: $L^2$-consistency of the fitted \emph{function} does not imply convergence of the selected \emph{structure} $\shat_m$ to $s_0$ unless $s_0$ is separated from all competitors by a positive risk margin, an assumption we do not make (and which our simulations show can fail: the empirical modal frequency of the outcome structure is often $\approx 0.4$--$0.5$, i.e.\ $p_0 < 1/2$).

\paragraph{Step 2: The selected structure inherits the rate (part b).}
Any selection rule with $s^* \in \{\shat_1,\dots,\shat_M\}$ satisfies $R(s^*) - R(s_0) \le \max_{m\le M}\{R(\shat_m) - R(s_0)\}$. By part (a) each term is $O_p(n^{-2\beta/(2\beta+d)})$; a union bound over $M = o(n)$ splits inflates the rate only by a $\log M$ factor absorbed into the $O_p$, giving $R(s^*) - R(s_0) = O_p(n^{-2\beta/(2\beta+d)})$.

\paragraph{Step 3: Risk closeness to prediction closeness (part c).}
Apply the risk-to-norm direction of the loss--norm link (Assumption~\ref{assump:regularity}; Lemma~\ref{lem:loss-norm}) with $\delta = R(s^*) - R(s_0)$:
\[
\|f_{s^*} - f_{s_0}\|_{L^2(\Prob)}^2 \le C_{\mathrm{ln}}\bigl(R(s^*)-R(s_0)\bigr) = O_p\!\left(n^{-2\beta/(2\beta+d)}\right). \qedhere
\]
\end{proof}

\begin{remark}[Why structure identity is neither achievable nor needed]
The previous statement of this theorem claimed modal-structure identity, $\Prob(s^* = s_0)\to 1$, and derived it from $L^2$-consistency of the fitted function. That inference is invalid without a risk margin separating $s_0$ from competing structures: many distinct partitions can achieve near-identical population risk, so the fitted function converges while the selected structure identity does not. Our simulations confirm this regime is common---on the complex outcome nuisance the modal frequency is $\approx 0.4$--$0.5$, so $p_0 < 1/2$ and the old finite-sample bound (which \emph{assumed} $p_0 > 1/2$) is vacuous. The repair is to weaken the claim to near-oracle \emph{risk}, hence near-oracle \emph{predictions} via the loss--norm link. This is exactly what the downstream results consume: Theorem~\ref{thm:pointwise-convergence} needs $\|f_{s^*}-\mu_0\|_{L^2}\to 0$ and Theorem~\ref{thm:inference} needs the product-rate condition, both of which follow from part (c) without any structure-identity claim. The benign empirical behavior---M-split matching cross-fitting even when the mode is diffuse---is explained precisely: near-tied-in-risk structures are near-equal in predictions, so which one the rule selects does not matter for inference. The cost is that the ``single interpretable tree'' is now justified as \emph{a} near-oracle representative rather than \emph{the} unique oracle structure; when interpretation of the specific partition matters, the diffuse-mode diagnostic (modal frequency) should be reported.
\end{remark}
...
\begin{proof}
\textbf{Proof roadmap:} We decompose the error into bias (from using $s^*$ rather than $\mu_0$) and variance (from averaging). By Theorem~\ref{thm:structure-consistency}(c), the selected structure has near-oracle predictions, $\|f_{s^*}-f_{s_0}\|_{L^2}=O_p(n^{-\beta/(2\beta+d)})$, so the structure-selection contribution to the bias vanishes at the tree rate---without requiring $s^*=s_0$. Tree approximation bias is $O_p(n^{-\beta/(2\beta+d)})$ by existing theory. Averaging $M$ independent refits reduces variance by a factor of $M$.
...
By Theorem~\ref{thm:structure-consistency}(c), $\|s^* - s_0\|_{\text{approx}} = \|f_{s^*}-f_{s_0}\|_{L^2(\Prob)} = O_p(n^{-\beta/(2\beta+d)})$ (near-oracle predictions), so this term is already at the tree rate and no structure-identity claim is needed. By tree approximation theory (e.g., Blanchard et al.\ 2007, Theorem~1), for trees with $s_n = n^{d/(2\beta+d)}$ leaves approximating a H\"older-$\beta$ function:
\[
...
Theorem~\ref{thm:functional-consistency} provides a rigorous, finite-sample guarantee that averaged predictions are functionally consistent: observations with identical covariates receive nearly identical predictions. Combined with Theorem~\ref{thm:structure-consistency} (the selected structure is near-oracle in risk, hence in predictions), this justifies the claim:

\begin{quote}
\textit{``We report one tree (structure $s^*$) whose predictions are $\varepsilon$-consistent with probability $1 - \delta$, and whose risk is within $O_p(n^{-2\beta/(2\beta+d)})$ of the oracle optimal structure.''}
\end{quote}

This is a much stronger claim than standard doubletree, which fixes a structure but leaves leaf values varying across folds without bounds. Note the claim is about near-oracle \emph{risk/predictions}, not identity of the specific partition: when several partitions have near-equal risk, $s^*$ is \emph{a} near-oracle representative, and its modal frequency should be reported as a stability diagnostic.
\end{remark}
...
\textbf{(d) Near-oracle risk:} M-split's selected structure is near-oracle in risk, hence in predictions via the loss--norm link (Theorem~\ref{thm:structure-consistency}), without requiring recovery of a unique oracle structure. Single-split selects one structure without such a guarantee.
...
\item \textbf{Structure selection criterion:} We use modal selection ($s^* = \text{mode}$). Alternatives: first structure ($s^* = \shat_1$), lowest cross-validated risk, or majority voting. Do these affect finite-sample performance?

\textit{Finding:} By Theorem~\ref{thm:structure-consistency}, all such rules yield near-oracle risk (any $s^*\in\{\shat_m\}$ inherits the rate), so all are asymptotically equivalent for inference. Simulations confirm this: on a complex outcome nuisance where the modal frequency is only $\approx 0.4$--$0.5$ (no dominant structure), modal and lowest-cross-validated-risk selection give statistically indistinguishable ATT bias, variance, and coverage---and both match cross-fitting. Selection matters for \emph{which} interpretable tree is reported, not for validity.
...
\item Main claims: selected structure is near-oracle in risk/predictions (Theorem~\ref{thm:structure-consistency}), predictions are functionally consistent (Theorem~\ref{thm:functional-consistency}), inference is valid (Theorem~\ref{thm:inference}).
...
\begin{enumerate}
\item \textbf{Near-oracle risk:} The selected structure $s^*$ has risk---and hence predictions, via the loss--norm link---within $O_p(n^{-2\beta/(2\beta+d)})$ of the oracle optimal structure, without requiring recovery of a unique oracle structure (Theorem~\ref{thm:structure-consistency}).
