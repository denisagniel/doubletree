...
% Its claim (piecewise-CONSTANT trees attain the s^{-2β/d} rate for β>1) is FALSE:
% constant-leaf trees saturate at first-order smoothness (Xu 2026, Remark 4.3),
% and the "remainder dominates" step below reverses the asymptotics. The theory
% now works in the anisotropic Besov regime with α<=1 (no β>1 case); see
% lem:approx in _pshab_appendix_draft.tex. Retained only for history; do not cite.
