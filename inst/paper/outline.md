...
| 2.2 | Covariate space, tree partitions, and optimal trees (`sec:trees`) | **Drafted 2026-09-21.** Unchanged existing content, plus: new numbered `ass:sparsity` (`manuscript.tex:123`), stating $\cS_e\ne\emptyset,\cS_\mu\ne\emptyset$ formally for the first time, placed immediately after `def:sufficient`; the legibility-vs-audit-completeness remark (Table 3, accepted) immediately follows it (`:139`), restating `ass:sparsity` by label, not just in spirit. Four of the six unlabeled prose restatements this was meant to collapse now cite `ass:sparsity` by label (`:263,:284,:358,:488`). **Correction, 2026-09-21:** the other two (`:711,:739`, inside `lem:single-tree-linear`/`cor:single-tree-coarsening`) are one-sided ($\cS_\mu\ne\emptyset$ alone, or added to individually) rather than the joint condition — substituting `\ref{ass:sparsity}` there would overstate what those results actually assume, so they were correctly left as direct $\cS_e/\cS_\mu$ prose; the original "six locations, one collapse" framing undercounted this. Also found already drafted, not newly added by this pass (contra this row's earlier text): the forward cross-reference at `:96` ("it is not merely a convention") pointing to §3.4/`sec:finite-discussion` predates this session. | `ass:sparsity` (`claims.md` Table 1, updated) | — | §2.1 | — |
