...
| 5 | Discussion (`sec:discussion`) | Renumbered from old §4. Unchanged existing content (returns to payment/triage/sentencing; audit-trail framing; limitations; future work), **plus two limitations `structure-reviewer` found omitted**: (i) `prop:spectest`'s size/power is unvalidated (`claims.md`: GAP), yet old `:699` offers it as the practitioner's operational decision rule with no caveat; (ii) old `:699`'s claim that the audit trail is "available whether or not structural sparsity actually holds" needs narrowing — when sparsity fails, the reported interval's *width* depends on a possibly non-interpretable, cross-fit anchor, which is exactly the non-auditability §1 says existing methods suffer from. Narrow the sentence, don't delete the point about the point estimate itself (which remains true). **New, decided 2026-09-18, not yet drafted:** (iii) a remark revisiting the ATT-vs-ATE choice, citing `hahn1998`, pointing to the new Appendix's ATE-specific results, and noting explicitly which parts of the paper (the single-tree family) are ATT-only. **New, decided 2026-09-17, not yet drafted:** (iv) the trees-vs-linear-models representational trade-off + its concrete mechanism ("a leaf for every configuration of the active covariates") — this content was removed from both the Introduction and old §3.6 during sequential edits and currently lives nowhere in the manuscript; author has decided it belongs in the Discussion, not the Introduction. | (prose, `sec:discussion` row in `claims.md`) | — | §1, §3, §4 | Renumbered from old §4 |
| Appendix | — | Empty in `manuscript.tex` itself (`\appendix` at `:703` precedes only the bibliography). Proof content lives in the separate companion document `theory.tex`, out of scope for this backfill. **New, decided 2026-09-18, not yet drafted:** will hold ATE-specific results. Decision: ATT stays the paper's primary estimand throughout; the main machinery (selection consistency, the CLT, the anchor interval) is noted as generalizing to the ATE, but the paper does not commit to full generality everywhere — specifically, the single-tree corollary family (§3.5) and `rem:two-trees-why`'s justification stay **ATT-only**, since both lean on `hahn1998`'s asymmetry (knowing $e_0$ exactly lowers the efficiency bound for the ATT but not the ATE) — not re-derived for ATE. | — | — | — | — |
