#!/bin/bash
#SBATCH --job-name=approach6
#SBATCH --output=logs/approach6_%a.out
#SBATCH --error=logs/approach6_%a.err
#SBATCH --array=1-600
...
# Approach 6: M-split Averaged (modal structure + averaged M×K leaf values)
# 600 jobs = 12 configs × 50 batches; 20 reps per batch = 1000 reps per config
...
BATCH=$(( (JOB_ID - 1) % N_BATCHES + 1 ))        # 1..50
