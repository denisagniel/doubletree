#' ATT Estimation with Linear Nuisances (Logistic Regression)
#'
...
#'
#' Expected behavior: Should perform well when nuisances are approximately
#' linear, but suffer bias when they are highly nonlinear (DGPs 2-3).
#'
...
#'   - convergence: "converged" or "warning" if glm had issues
#'
#' @details
...
#' fit <- att_linear(d$X, d$A, d$Y, K = 5, seed = 123)
#' print(fit$theta)
#' print(fit$ci)

att_linear <- function(X, A, Y, K = 5, seed = NULL,
  ...
  for (k in 1:K) {
    ...
    if (interactions && ncol(X) <= 4) {
      ...
      formula_e <- as.formula("A ~ .")
    }

    glm_e <- tryCatch({
      glm(formula_e, data = train_data_e, family = binomial())
    }, warning = function(w) {
      ...
      glm(A ~ ., data = train_data_e, family = binomial())
    })

    # Predict on test fold
    pred_e <- predict(glm_e, newdata = X[test_idx, , drop = FALSE],
                      type = "response")
    e_hat[test_idx] <- pred_e
  ...
  result <- list(
    ...
    ci = ci,
    n_treated = n_treated,
    convergence = if (convergence_issues) "warning" else "converged",
    method = "linear",
    hyperparams = list(
  ...
  if (verbose) {
    ...
    if (convergence_issues) {
      cat("  Warning: GLM convergence issues detected\n")
    }
  }
