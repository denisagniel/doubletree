...
for (i in 1:nrow(results)) {
  ...
  # Fit propensity tree
  tryCatch({
    e_model <- optimaltrees::fit_tree(X_train, A_train,
  ...
  if (length(idx0_train) > 10) {
    tryCatch({
      m0_model <- optimaltrees::fit_tree(X_train[A_train == 0, , drop = FALSE],
                                       Y_train[A_train == 0],
                                       loss_function = "log_loss",
                                       regularization = lambda,
                                       verbose = FALSE)
  ...
  if (length(idx1_train) > 10) {
    tryCatch({
      m1_model <- optimaltrees::fit_tree(X_train[A_train == 1, , drop = FALSE],
                                       Y_train[A_train == 1],
                                       loss_function = "log_loss",
                                       regularization = lambda,
                                       verbose = FALSE)
