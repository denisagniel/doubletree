...
#
# Uses doubletree::estimate_att() with optimaltrees for nuisance trees.
...
result <- doubletree::estimate_att(data$X, data$A, data$Y, K = K)
...
for (i in seq_len(n_sim)) {
  ...
  results_dgp1[[i]] <- doubletree::estimate_att(d$X, d$A, d$Y, K = K)
}
...
for (i in seq_len(n_sim)) {
  ...
  results_dgp2[[i]] <- doubletree::estimate_att(d$X, d$A, d$Y, K = K)
}
