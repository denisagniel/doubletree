...
#
# Run:  Rscript simulations/head_to_head_comparison/slurm/run_single_replication.R \
#         --regime R2 --task-id 7 --pkg-root /path/to/doubletree --scratch-dir /path/to/scratch
#
# ONE SLURM ARRAY TASK'S WORTH OF WORK: one contiguous block of replications of
# one (regime, dgp, n) cell, as mapped by slurm/units.R. Writes ONE shard to the
# run's scratch directory; slurm/combine_results.R reassembles shards into the
# cell_*.rds checkpoints that code/analyze.R already reads.
#
...
# (one-replication tasks would spend more wall time starting R than estimating,
# and whole-cell tasks would floor the wall clock at R2's 11.6 h n = 8000 cell).
#
# ------------------------------------------------------------
...
#
# common.R does much more than load packages -- it defines the DGP registry, the
# arm registry, the regime grid, the seed scheme and run_one_rep()/run_cell() --
# so it is sourced in full here rather than being partially copied. Copying the
# study logic into the SLURM script would create a second definition of the
# experiment that could drift from the one the pilot validated.
#
# ------------------------------------------------------------
...
#  * on_error = "record": a failed replication persists ONE ROW PER ARM carrying
#    the condition message in `error_message`, per common.R's rep_row_template().
...
# (SEED_MASTER, regime, dgp, n, rep), so replication r of a cell is byte-identical
# whether it runs here as part of batch 3 or in a sequential run_sweep(). Batching
...
# Identical to run_sweep.R. Without it a multithreaded BLAS makes spec §5's
# per-method wall-clock comparison meaningless AND makes the memory footprint
# unpredictable (simulations/docs/MEMORY_SAFE_SIMULATIONS.md). On a cluster it
# additionally means one task can oversubscribe its single allocated CPU.
...
# <pkg root>/simulations/head_to_head_comparison/slurm -> up three levels.
...
#' Verify optimaltrees/doubletree were actually loaded from an installed
#' library, not dev-loaded from source.
#'
#' AND THE PATH IS VERIFIED, NOT ASSUMED. The doubletree repository ships a
#' tracked `.Rprofile` that runs devtools::load_all("../optimaltrees") on
#' startup, so on a side-by-side checkout an R process can end up with a
#' DEV-LOADED optimaltrees even though this script asked for the installed
#' one, and every result would then have been produced by uninstalled source.
#' That is exactly the silent-wrong-answer failure this gate exists to
#' prevent, so it is checked and it aborts rather than warns.
#'
#' @return Named character vector of each namespace's resolved directory.
...
option_list <- list(
  make_option("--regime", type = "character", dest = "regime", default = "R6",
              help = "Regime id, one of R1..R6 [default %default]"),
  make_option("--task-id", type = "integer", dest = "task_id", default = 1L,
              help = "SLURM array task id within this regime's array, 1-based [default %default]"),
  ...
  make_option("--scratch-dir", type = "character", dest = "scratch_dir",
              default = file.path(tempdir(), "h2h_scratch"),
              help = "Directory for this run's result shards [default %default]"),
  make_option("--target-secs", type = "double", dest = "target_secs",
              default = NA_real_,
              help = "Target wall cost per task, in seconds; MUST match the launcher's value or the array map differs [default: H2H_TARGET_SECS, else 1800]"),
...
if (!interactive() && sys.nframe() == 0L) {
  ...
  if (!isTRUE(USE_INSTALLED_PKGS)) {
    stop("common.R took the SOURCE loading path despite H2H_USE_INSTALLED = 1; ",
  ...
  ut <- h2h_unit_table(opt$regime, target_secs = target_secs)
  if (opt$task_id > nrow(ut)) {
    # Array padding beyond the regime's task count. Not an error: exit 0 so SLURM
    # records the task as done rather than failed.
    cat(sprintf("[%s task %d] beyond this regime's %d task(s); nothing to do.\n",
                opt$regime, opt$task_id, nrow(ut)))
  ...
  # a smoke-test shard can never be mistaken for a production one.
  ...
  # costs O(reps^2 / cadence) bytes written: measured on R6's 2000-replication
  # block, flushing every 25 replications (80 flushes) took 27.8 s against 24.0 s
  # with a single flush -- 16 % overhead to buy crash granularity the block does
  # not need, since the whole block is 24 s. Capping the block at ~20 flushes
  # keeps the overhead near 1 % and still bounds a timeout's loss at 5 % of a
  # task. An explicit --flush-every always wins, for tests that want per-rep
  # flushes.
  ...
  shard <- file.path(opt$scratch_dir,
                     batch_filename(u$regime, u$dgp, u$n, u$reps_total, u$batch))
  partial <- file.path(opt$scratch_dir,
                       partial_filename(u$regime, u$dgp, u$n, u$reps_total, u$batch))

  cat(sprintf(
    paste0("[%s task %d/%d] dgp=%s n=%d reps %d-%d of %d (batch %d/%d), ",
           "projected %.1f s, target %.0f s, flush every %d\n"),
    u$regime, opt$task_id, nrow(ut), u$dgp, u$n, u$rep_from, u$rep_to,
    u$reps_total, u$batch, u$n_batches, u$est_secs, target_secs, flush_every))
  cat(sprintf("[%s task %d] host=%s R=%s doubletree=%s optimaltrees=%s\n",
              u$regime, opt$task_id, Sys.info()[["nodename"]],
              getRversion(), utils::packageVersion("doubletree"),
              utils::packageVersion("optimaltrees")))

  # ---- 4 idempotent skip -------------------------------------------------
  if (file.exists(shard) && !opt$force) {
    cat(sprintf("[%s task %d] shard already exists (%s); skipping.\n",
                u$regime, opt$task_id, basename(shard)))
    quit(save = "no", status = 0)
  }
  ...
  if (file.exists(partial) && !opt$force) {
    p <- readRDS(partial)
    same_unit <- identical(p$unit$regime, u$regime) &&
    ...
    if (same_unit) {
      ...
      cat(sprintf("[%s task %d] resuming: %d replication(s) already flushed; next rep = %d.\n",
                  u$regime, opt$task_id, length(unique(p$results$rep)), next_rep))
    } else {
      cat(sprintf("[%s task %d] partial %s belongs to a different unit; ignoring it.\n",
                  u$regime, opt$task_id, basename(partial)))
    }
  }
  ...
  if (next_rep <= u$rep_to) {
    for (r in seq.int(next_rep, u$rep_to)) {
      done_rows[[length(done_rows) + 1L]] <-
        run_one_rep(u$regime, dgp, u$n, r, on_error = "record")
      ...
      if (n_since_flush >= flush_every || r == u$rep_to) {
        ...
        cat(sprintf("[%s task %d] rep %d/%d flushed (%.1f s elapsed)\n",
                    u$regime, opt$task_id, r, u$rep_to,
                    as.numeric(difftime(Sys.time(), t0, units = "secs"))))
      }
    }
  } else {
    cat(sprintf("[%s task %d] every replication already flushed; assembling shard.\n",
                u$regime, opt$task_id))
  }

  res <- do.call(rbind, done_rows)
  res <- res[order(res$rep, match(res$arm, REGIMES[[u$regime]]$arms)), , drop = FALSE]
  rownames(res) <- NULL
  ...
  if (!identical(as.integer(got), as.integer(want))) {
    stop(sprintf("[%s task %d] shard would hold reps {%s} but the block is %d-%d.",
                 u$regime, opt$task_id, paste(got, collapse = ","),
  ...
  write_atomic(list(
    ...
    runtime = list(
      ...
      secs_in_arms = sum(res$secs, na.rm = TRUE),
  ...
  cat(sprintf("[%s task %d] wrote %s -- %d reps x %d arms = %d rows, %d failed rep(s), %.1f s (projected %.1f s)\n",
              u$regime, opt$task_id, basename(shard), u$reps,
              length(REGIMES[[u$regime]]$arms), nrow(res), n_fail,
              elapsed, u$est_secs))

  # Distinct error texts once per shard: a per-arm failure rate is recoverable
  # from the rows, but the MESSAGE is what says whether to fix or resubmit.
  em <- unique(stats::na.omit(res$error_message))
  if (length(em)) {
    cat(sprintf("[%s task %d] %d distinct error message(s):\n",
                u$regime, opt$task_id, length(em)))
