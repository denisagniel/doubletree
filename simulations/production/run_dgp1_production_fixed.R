...
run_single_sim <- function(sim_id, grid, dgps, tau, k_folds, seed_offset) {
  ...
  result <- tryCatch({
    if (row$method == "tree") {
      ...
      fit <- suppress_all({
        method_forest_dml(X = d$X, A = d$A, Y = d$Y, K = k_folds)
      })
      list(theta = fit$theta, sigma = fit$sigma,
           ci_lower = fit$ci_lower, ci_upper = fit$ci_upper,
           converged = TRUE, epsilon_n = NA)

    } else if (row$method == "linear") {
      fit <- suppress_all({
        method_linear_dml(X = d$X, A = d$A, Y = d$Y, K = k_folds)
      })
      list(theta = fit$theta, sigma = fit$sigma,
           ci_lower = fit$ci_lower, ci_upper = fit$ci_upper,
           converged = TRUE, epsilon_n = NA)
