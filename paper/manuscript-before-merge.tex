...
\begin{equation}
\label{eq:score}
\psi(O; \theta, \eta) = \frac{A}{\pi}\bigl( Y - \mu_0(\bX) - \theta \bigr) - \frac{1}{\pi} \cdot \frac{e(\bX)(1-A)}{1 - e(\bX)} \bigl( Y - \mu_0(\bX) \bigr)
\end{equation}
...
\thetahat = \frac{1}{n} \sum_{i=1}^n \Biggl[ \frac{A_i}{\hat\pi}\bigl( Y_i - \hat\mu_0(\bX_i) \bigr) - \frac{1}{\hat\pi} \cdot \frac{\hat{e}(\bX_i)(1-A_i)}{1 - \hat{e}(\bX_i)} \bigl( Y_i - \hat\mu_0(\bX_i) \bigr) \Biggr],
\end{equation}
...
\begin{remark}[Cross-validation for penalty selection]
\label{rem:cv-lambda}
In practice, the penalty parameter $\lambda_n$ is typically chosen by cross-validation rather than set to the theoretical rate $(\log n)/n$. The validity of this approach relies on the oracle property of cross-validation: under appropriate conditions, the CV-selected penalty $\hat{\lambda}_{\text{CV}}$ achieves the same rate as the oracle choice up to constant factors \citep{arlot2009data, gyorfi2002distribution}. Specifically, for penalized complexity-based model selection in regression, cross-validation has been shown to adaptively select penalties that balance approximation and estimation error, yielding $\hat{\lambda}_{\text{CV}} = \Theta_p((\log n)/n)$ asymptotically \citep{yang2007consistency}. While a formal proof for decision trees with leaf-count penalization remains an open question, our simulation studies (Section~\ref{sec:results}) empirically verify that CV-selected penalties yield convergence rates consistent with the theoretical predictions.
\end{remark}
...
\begin{remark}[Practical choice of $\varepsilon_n$]
\label{rem:epsilon-choice}
The tolerance $\varepsilon_n$ must satisfy $\varepsilon_n = o(n^{-1/2})$ for DML validity. We recommend:

\textbf{1. Fixed default.} Set $\varepsilon_n = c\sqrt{(\log n)/n}$ for $c \in \{1,2,3\}$. This is conservative relative to the empirical process fluctuation scale and satisfies the rate requirement.

\textbf{2. Adaptive scaling.} Set
\[
\varepsilon_n = \sqrt{\frac{\log n}{n}} \cdot \max\Bigl(1, \frac{\text{sd}(\text{best}^{(-1)}, \ldots, \text{best}^{(-K)})}{\text{median}(\text{best}^{(-1)}, \ldots, \text{best}^{(-K)})}\Bigr).
\]
This adapts to the observed coefficient of variation across folds while preserving $\varepsilon_n = o_p(n^{-1/2})$.

\textbf{3. Grid search (asymptotically valid).} Pre-specify a grid
\[
\varepsilon_n \in \Bigl\{c_1\sqrt{\frac{\log n}{n}}, c_2\sqrt{\frac{\log n}{n}}, \ldots, c_M\sqrt{\frac{\log n}{n}}\Bigr\}
\]
where $0 < c_1 < c_2 < \cdots < c_M$ (e.g., $c_j \in \{1, 2, 3, 5, 10\}$). Select the smallest $\varepsilon_n$ yielding non-empty intersection. Since all grid points satisfy $o(n^{-1/2})$ by construction, data-dependent selection among them is asymptotically valid: under the stability condition (A4), fold-specific minimizers converge to similar structures as $n \to \infty$, so the grid search selects $\varepsilon_n \to 0$, yielding the tightest valid Rashomon set.

\textbf{Finite-sample guidance.} If the intersection is empty for all $\varepsilon_n \le 0.2$, this indicates substantial cross-fold heterogeneity; revert to fold-specific trees rather than forcing a common structure with large $\varepsilon_n$. The bias penalty for using an intersection tree is $O_p(\varepsilon_n)$ (Corollary~\ref{cor:intersection-overhead}), so $\varepsilon_n$ balances interpretability against statistical efficiency.
\end{remark}
