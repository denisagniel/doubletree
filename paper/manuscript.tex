...
\end{assumption}
\begin{assumption}[Positivity] (We require strong positivity here, I think, because of the Norm-loss link)
\end{assumption}
\begin{assumption}[Exchangeability]
\end{assumption}
These assumptions are standard in the causal inference literature. Consistency states that there aren't multiple versions of treatment, and we observe one of the counterfactuals. Positivity bounds the propensity score and ensures sufficient overlap between treated and control groups. Note that typically the ATT requires a weaker form of positivity, but we assume a stronger form for later proofs (cite appendix lemma). Exchangeability ensures that there is no unmeasured confounding. 
...
The following oracle inequality is a direct application of \citet{xuStatisticalOptimalityOptimal2026}, Theorem 3.1, to our DML setting. We state it here for completeness and to establish notation for subsequent results.
