...
\begin{abstract}
    Most causal inference focuses exclusively on eliminating bias when estimating causal quantities. Some settings require a focus on interpretability in addition to bias reduction because of the high-stakes nature of the application of causal techniques. For these cases, we introduce a fully interpretable and nonparametric method for estimating causal effects for high-stakes settings. Our approach takes advantage of recent advances in the estimation and understanding of optimal decision trees. Nuisance functions (propensity scores and outcome models) are estimated via optimal trees and are thus intepretable in terms of simple decision rules. We show that optimal trees can satisfy the rate conditions required for incorporation into augmented inverse propensity weighting/double machine learning estimators via cross-fitting, and we outline the conditions on tuning parameter selection and smoothness of underlying nuisance functions that yield asymptotic normality of resulting causal effect estimates. Further, we provide an alternative to cross-fitting that results in a single nuisance function tree for the entire dataset, unlike cross-fitting which has different nuisance function estimators for different folds of the dataset. Thus, in practice, effect estimates can be explained entirely by two nuisance trees. 
\end{abstract}
...
Estimation of causal effects in high-dimensional and flexible modeling environments relies on semiparametric methods using efficient influence functions (EIFs), which combine machine learning with orthogonalized score functions to produce asymptotically normal, root-$n$ consistent estimates even when complex models are used to estimate nuisance functions \citep{chernozhukov2018}. A key technical requirement is that nuisance estimators converge sufficiently fast---typically faster than $n^{-1/4}$ in mean squared error---so that their estimation error is effectively second-order in the orthogonal identifying equations. In practice, off-the-shelf machine learning methods such as random forests, gradient boosting, and neural networks are commonly employed in machine learning-based causal inference \citep{bach2021, susmann2025}, including for healthcare provider evaluation. While these flexible learners often achieve good predictive performance, they are generally black-box models whose internal structure cannot be easily interpreted. Interpretability can be critical in many application domains where stakeholders demand transparent models that offer clear decision rules or partitions of the covariate space.

Optimal decision trees---including recent advances such as Generalized and Scalable Optimal Sparse Decision Trees (GOSDT)---offer a compelling trade-off: they are fully interpretable, provide globally optimized decision rules, and avoid some of the pathologies of greedy tree induction \citep{lin2020}. These optimal trees have been studied from a computational and predictive performance perspective, but their statistical properties in semiparametric inference contexts have received less theoretical attention. Several related strands of literature inform the theoretical development of tree-based estimation: \citet{nobel1996histogram} established general consistency results for data-dependent partitions including regression trees; \citet{chatterjee2019adaptive} derived oracle inequalities and risk bounds for optimal dyadic decision tree estimators in bounded variation and piecewise polynomial function classes; \citet{bertsimas2017optimal} studied globally optimal classification trees with sparsity constraints and established approximation guarantees for piecewise polynomial target functions; and \citet{mourtada2018minimax} proved minimax-optimal rates for Mondrian trees and forests, a specific class of random partition-based estimators. In the Bayesian framework, \citet{vanderpas2017bayesian} established near-minimax posterior concentration rates for dyadic tree priors. However, these results do not directly verify that globally optimal decision trees satisfy the stringent $o_p(n^{-1/4})$ condition required for valid inference with EIF-based estimators.

In concurrent work, \citet{xuStatisticalOptimalityOptimal2026} established comprehensive statistical theory for empirical risk minimization (ERM) decision trees under random design in both regression and classification settings. They derive sharp oracle inequalities that bound excess risk relative to the best tree approximation with at most $L$ leaves, characterizing the interpretability-accuracy trade-off. They prove minimax-optimal rates over piecewise sparse heterogeneous anisotropic Besov (PSHAB) function classes, which capture sparsity, anisotropic smoothness, and spatial heterogeneity. Their framework introduces a novel uniform concentration approach based on empirically localized Rademacher complexity, providing theoretical tools broadly applicable beyond decision trees. These results establish the foundational statistical guarantees for optimal trees in prediction settings. Our work complements theirs by: (1) verifying that their rates satisfy the stringent requirements for valid EIF-based causal inference; (2) deriving the explicit condition $\beta > d/2$ for achieving the necessary convergence rates; (3) introducing the Rashomon set intersection approach for interpretable reporting across cross-validation folds; and (4) providing practical implementation and validation in high-stakes healthcare applications.

\textbf{In this paper, we bridge optimal tree theory to causal inference.} Building on the oracle inequality framework established by \citet{xuStatisticalOptimalityOptimal2026}, we verify that optimal decision tree estimators achieve rates sufficient for valid EIF-based causal inference. Specifically, we derive the condition $\beta > d/2$ under which the minimax rate $n^{-\beta/(2\beta+d)}$ satisfies the $o_p(n^{-1/4})$ requirement for valid semiparametric inference, enabling root-$n$ consistent and asymptotically normal estimation of treatment effects with interpretable nuisance functions. Beyond this theoretical verification, we introduce a novel \textbf{Rashomon set intersection approach} for selecting a single interpretable tree structure that remains near-optimal across cross-validation folds, enabling both theoretical guarantees and practical interpretability. Our contributions are threefold:

\textbf{1. Application of optimal tree theory to EIF-based causal inference.} Building on the recent oracle inequality results of \citet{xuStatisticalOptimalityOptimal2026}, who establish that globally optimal penalized decision trees achieve minimax rates over PSHAB function classes (including Hölder spaces), we verify that these rates satisfy the stringent requirements for valid causal inference with efficient influence functions. Our key contributions: (a) we derive the explicit condition $\beta > d/2$ under which the tree rate $n^{-\beta/(2\beta+d)}$ satisfies the $o_p(n^{-1/4})$ requirement for valid semiparametric inference, establishing that interpretable tree-based nuisance estimators enable $\sqrt{n}$-consistent and asymptotically normal causal inference; (b) we provide complete proofs connecting tree approximation theory, oracle inequalities, and the semiparametric rate condition; and (c) we verify the loss-norm link required for the EIF expansion under both squared loss and cross-entropy.

\textbf{2. Methodological innovation: Rashomon set intersection.} We introduce a principled approach for selecting a single interpretable tree structure across cross-validation folds while preserving theoretical guarantees. By taking the intersection of Rashomon sets (near-optimal solutions) across folds, we identify tree structures that remain robustly good when refit on different training data. This enables reporting of a single, interpretable decision rule to stakeholders while maintaining the cross-fitting structure essential for valid inference. We characterize conditions under which this intersection is non-empty and verify them empirically.

\textbf{3. Practical implementation and validation.} We develop software packages (optimaltrees and doubletree) implementing optimal tree estimation with Rashomon set computation, and provide comprehensive simulation studies demonstrating that interpretable trees within efficient influence function-based estimation achieve competitive estimation quality with transparent decision rules. We apply the methodology to healthcare quality measurement, illustrating its utility in high-stakes regulatory settings.
...
A central contribution of this work is to show that \emph{interpretable} learners (decision trees) can satisfy the convergence requirements for valid inference and thus deliver valid inference---transparent models need not sacrifice statistical validity. Tree-based EIF estimation is a candidate for settings like Medicare quality measurement: interpretable nuisances, valid confidence intervals, and transparent rules. Caveats include the need for validation, domain input, and regulatory alignment.

\section{Conclusion}
\label{sec:conclusion}
We addressed the lack of formal rate guarantees for interpretable nuisance learners in EIF-based causal inference. We showed that optimal decision trees, used as nuisance estimators for the ATT, can satisfy the $o(n^{-1/4})$ rate under identified conditions on smoothness and complexity, and we used Rashomon set intersection to select a single reportable tree per nuisance while preserving valid $\sqrt{n}$-inference. A general theorem (Appendix) guarantees that any fold-specific nuisances that are $o_p(n^{-1/4})$-close to the truth yield the same asymptotic normality.
