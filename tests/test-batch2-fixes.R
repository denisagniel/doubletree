...
result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, verbose = "yes", regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, stratified = 1, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, cv_regularization = "TRUE", regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

# (The former `use_rashomon = 1` logical-validation case is gone: `use_rashomon` no
# longer exists as a parameter. The Rashomon path is now a separate function,
# estimate_att_rashomon(), so the flag cannot be mistyped.)

result <- tryCatch({
  estimate_att_rashomon(X, A, Y, K = 3, auto_tune_intersecting = "false", regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")
...
result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, seed = "abc", regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, seed = TRUE, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")
...
result <- tryCatch({
  estimate_att_rashomon(X, A, Y, K = 3, max_leaves = 0, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

result <- tryCatch({
  estimate_att_rashomon(X, A, Y, K = 3, max_leaves = 2.5, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

result <- tryCatch({
  estimate_att_rashomon(X, A, Y, K = 3, max_leaves = -1, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")
...
result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3.5, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")
...
result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, discretize_method = "invalid", regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")

result <- tryCatch({
  estimate_att_crossfit(X, A, Y, K = 3, discretize_method = "mean", regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")
...
result <- tryCatch({
  estimate_att_rashomon(X, A, Y, K = 3, rashomon_bound_adder = -0.1, regularization = 0.1)
  "NO_ERROR"
}, error = function(e) "ERROR")
stopifnot(result == "ERROR")
...
fit <- estimate_att_crossfit(X, A, Y, K = 3, regularization = 0.1, verbose = FALSE,
                    seed = 42, discretize_method = "quantiles")
